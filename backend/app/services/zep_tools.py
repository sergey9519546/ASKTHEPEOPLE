...
Encapsulates graph search, node reading, edge querying, etc., for use by the Report Agent.
...
2. PanoramaSearch (Wide Search) - Get full picture, including expired content.
...
class InsightForgeResult:
    ...
    Includes retrieval results for multiple sub-queries and comprehensive analysis
    """
    query: str
    ...
    def to_text(self) -> str:
        ...
        if self.key_quotes:
            ...
            for quote in self.key_quotes:
                ...
                # Filter junk content containing question numbers (Questions 1-9)
                skip = False
                for d in '123456789':
                ...
                if skip:
                    continue
                # Truncate excessively long content (truncate by period, not hard truncation)
...
class ZepToolsService:
    ...
    - get_node_detail - Get detailed info for a specific node
    - get_node_edges - Get edges associated with a specific node
    - get_entities_by_type - Retrieve entities by their type
    - get_entity_summary - Get relationship summary for a specific entity
    """
    ...
    # Class-level cache for graph data (shared across instances)
    _nodes_cache = {}
    _edges_cache = {}
    _cache_lock = threading.Lock()
    CACHE_TTL = 300  # 5 minutes Time-To-Live
    ...
    def search_graph(
        ...
        Search for relevant info in the graph using hybrid search (semantic + BM25).
        If Zep Cloud search API is unavailable, fallback to local keyword matching.
        
        Args:
            graph_id: Graph ID (Standalone Graph)
            query: Search query
            limit: Quantity of returned results
    ...
    def _local_search(
        ...
        Get all edges/nodes, then match keywords locally
        ...
        # Extracting search keywords (simple tokenization)
    ...
    def invalidate_cache(self, graph_id: str = None):
        """
        Invalidate cache for a specific graph or all graphs.
        """
        with self._cache_lock:
            if graph_id:
                keys_to_delete = [k for k in self._nodes_cache.keys() if k.startswith(graph_id)]
                for k in keys_to_delete:
                    del self._nodes_cache[k]
                keys_to_delete = [k for k in self._edges_cache.keys() if k.startswith(graph_id)]
                for k in keys_to_delete:
                    del self._edges_cache[k]
                logger.info(f"Invalidated cache for graph {graph_id}")
            else:
                self._nodes_cache.clear()
                self._edges_cache.clear()
                logger.info("Invalidated all graph cache")
    ...
    def get_all_nodes(self, graph_id: str) -> List[NodeInfo]:
        ...
        cache_entry = None
        with self._cache_lock:
            cache_entry = self._nodes_cache.get(cache_key)

        if cache_entry and time.time() - cache_entry['timestamp'] < self.CACHE_TTL:
            logger.info(f"Using cached nodes for graph {graph_id}")
    ...
    def get_all_edges(self, graph_id: str, include_temporal: bool = True) -> List[EdgeInfo]:
        ...
        cache_entry = None
        with self._cache_lock:
            cache_entry = self._edges_cache.get(cache_key)

        if cache_entry and time.time() - cache_entry['timestamp'] < self.CACHE_TTL:
            logger.info(f"Using cached edges for graph {graph_id}")
            return copy.deepcopy(cache_entry['data'])
    ...
    def get_node_detail(self, node_uuid: str) -> Optional[NodeInfo]:
        """
        Retrieve detailed info for a regular node
        ...
        Returns:
            Node info or None
        """
        logger.info(f"Get node detail: {node_uuid[:8]}...")
    ...
    def get_node_edges(self, graph_id: str, node_uuid: str) -> List[EdgeInfo]:
        ...
        Retrieve all edges of the graph, then filter those related to the specified node
    ...
    def get_entities_by_type(
        ...
        filtered = []
        for node in all_nodes:
            # Check if labels contains specified type
    ...
    def get_entity_summary(
        ...
        Search for all info related to the entity and generate summary
    ...
    def get_graph_statistics(self, graph_id: str) -> Dict[str, Any]:
        ...
        """
        logger.info(f"Retrieving graph statistics for {graph_id}...")
    ...
    def get_simulation_context(
        ...
        Comprehensive search for all info related to simulation requirements
    ...
    def insight_forge(
        ...
        3. Extracts relevant entities and retrieves their detailed info
        ...
        Args:
            ...
            query: User query
            simulation_requirement: Simulation requirement description
            report_context: Report context (optional, used for more precise sub-query generation)
        ...
        for uuid in list(entity_uuids):  # Process all entities, no truncation
            if not uuid:
                continue
            try:
                # Separately get info for each relevant node
        ...
        logger.info(f"InsightForge completed: Found {result.total_facts} facts, {result.total_entities} entities, {result.total_relationships} relationships")
        return result
...
Return the list of sub-queries in JSON format."""
    ...
    def panorama_search(
        ...
        3. Classify and organize currently active and historical info
        
        Suitable for scenarios where a complete view of events and tracking evolution is needed.
    ...
    def quick_search(
        ...
        logger.info(f"QuickSearch completed: Found {result.total_count} results")
        return result
    ...
    def _clean_tool_call_response(response: str) -> str:
        """Clean JSON tool call wrappers in Agent replies, extracting actual content"""
    ...
    def _load_agent_profiles(self, simulation_id: str) -> List[Dict[str, Any]]:
        ...
        # Priority: try reading Reddit JSON format
        ...
        if os.path.exists(twitter_profile_path):
            try:
                with open(twitter_profile_path, 'r', encoding='utf-8') as f:
                    ...
                    for row in reader:
                        # Convert CSV format to uniform format
                        profiles.append({
