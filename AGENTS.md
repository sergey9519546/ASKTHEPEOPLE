...
| 5 | Advanced simulation methodology | `askthepeople-ai-eval-steward` + `askthepeople-architect` | PARTIAL | Behavioural and analytical model layer exists under `backend/app/services/` (`big_five`, `prospect_theory`, `diffusion_model`, `constraint_engine`, `game_theory`, `calibration_metrics`) exported in `services/__init__.py` and verified by unit/eval suites. Remaining: model-release gating and adversarial/sensitivity eval suites |
