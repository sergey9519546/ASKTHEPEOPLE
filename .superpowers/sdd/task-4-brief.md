...
The source authority reconciliation is complete as a design decision. It must
still land in the normative documents, pass `python tools/validate_docs.py`,
and receive the named owner approvals before canonical writes or an enabled
worker use it. Until then, domain work remains `TRANSITION — DOMAIN ONLY`
behind the disabled flag. Do not preserve stale aliases for the superseded
relations or transitions.

### 6.1 Reconciled source state machine

Once the normative packet lands, this is the exact closed transition set:
...
Until this packet lands, candidate types may be developed only in the disabled
module and no new canonical provenance edge may be written.
...
Create `backend/app/domain/identifiers.py` for the repository-wide UUIDv7 and
public-ID contract, then create `backend/app/domain/source_ingestion.py` for
the source aggregate. Models are Pydantic v2 frozen, strict, and
...
Use one shared strict type and one injectable generator:
...
def require_uuid7(value: UUID) -> UUID:
    if value.version != 7 or value.variant != RFC_4122:
        raise ValueError("uuid7_required")
    return value

UUID7 = Annotated[UUID, AfterValidator(require_uuid7)]

class PublicIdPrefix(str, Enum):
    SOURCE = "src_"
    SOURCE_VERSION = "srcv_"
    SOURCE_SEGMENT = "seg_"
    CANDIDATE = "cand_"
    STARTING_CONDITION = "cond_"
    REVIEW_FLAG = "sflag_"
    REVIEW_EVENT = "srev_"
    DELETION_REQUEST = "del_"

class IdentifierFactory(Protocol):
    def new_physical_id(self) -> UUID7: ...
    def new_public_id(self, prefix: PublicIdPrefix) -> str: ...
```

`new_public_id` creates a fresh UUIDv7, verifies its version/variant, removes
hyphens, and prepends the enum-owned prefix. It never formats a supplied
physical ID. Tests inject deterministic clock/randomness seams; production
randomness is cryptographic.
...
After the reconciled normative packet in section 6.2 lands, the closed
allowlist uses these source-review edges. `EXTRACTED_FROM` is an existing
relation; only its candidate triple becomes newly authorized:
