...
const handleNextStep = (params = {}) => {
  if (currentStep.value < 5) {
    currentStep.value++;
    addLog(
      `进入 Step ${currentStep.value}: ${stepNames[currentStep.value - 1]}`,
    );
...
const handleGoBack = () => {
  if (currentStep.value > 1) {
    currentStep.value--;
    addLog(
      `返回 Step ${currentStep.value}: ${stepNames[currentStep.value - 1]}`,
    );
  }
};
...
.app-header {
  height: 60px;
  border-bottom: var(--border-width) solid var(--bau-black);
  display: flex;
  align-items: center;
...
.view-switcher {
  ...
  background: var(--bau-bg);
  padding: 4px;
  border: var(--border-width) solid var(--bau-black);
  gap: 0;
}
...
.panel-wrapper.left {
  /* Previously had border-right, now handled by gap + individual borders */
}
</style>
