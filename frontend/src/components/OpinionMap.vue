...
const latestOpinions = computed(() => {
  ...
  const sorted = [...opinions.value].sort(
    // ⚡ Bolt Optimization: Compare ISO 8601 strings directly instead of creating Date objects.
    // This prevents O(N log N) object allocations and reduces GC overhead during frequent reactive updates.
    (a, b) =>
