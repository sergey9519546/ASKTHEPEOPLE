...
const latestOpinions = computed(() => {
  ...
  const sorted = [...opinions.value].sort(
    // ⚡ Bolt: Optimize date comparison by comparing ISO 8601 strings directly instead of instantiating Date objects
    // Impact: Avoids expensive Date parsing inside sort loop, improving render performance for large opinion arrays
    (a, b) =>
