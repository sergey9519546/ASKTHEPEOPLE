...
const latestOpinions = computed(() => {
  ...
  const sorted = [...opinions.value].sort((a, b) => {
    /* ⚡ Bolt Performance Optimization: Compare ISO 8601 strings directly instead of instantiating Date objects. Impact: Avoids O(N log N) Date allocations. */ return a.timestamp <
      b.timestamp
      ? -1
